...
<div class="x-body">
	...
	<div class="layui-row">
		<form class="layui-form layui-col-md12 x-so">
			<input type="text" name="intoName" id="intoName" placeholder="请输入药品名称" autocomplete="off" class="layui-input"
				   lay-verify="required">
			<button class="layui-btn" lay-submit="" lay-filter="search-comment"><i class="layui-icon">&#xe615;</i></button>
		</form>
	</div>
	<xblock>
		<button class="layui-btn" onclick="x_admin_show('添加','./intoman-add.jsp',600,400)"><i
...
<script>
	layui.use(['table', 'form', 'layer','jquery','util'], function () {
		...
		table.render({
			...
			, elem: '#comment-table'
			, cellMinWidth: 40
			, url: '<%=request.getContextPath()%>/select/intoManList/intoMan' //数据接口，查询所有list
			...
			, cols: [[ //表头
				{type: 'checkbox'},
				{field: 'intoId', title: 'ID', sort: true, width: 68}//主键
				, {field: 'intoName', title: '药品名称'} //对应的实体的字段
				, {field: 'intoType', title: '数量'} //对应的实体的字段
				, {field: 'intoNumber', title: '种类'} //对应的实体的字段
				, {field: 'intoPrice', title: '进货单价'} //对应的实体的字段
				, {field: 'intoCreateTime', title: '进货时间',templet: function (data) {
					return  util.toDateString(data.intoCreateTime, "yyyy-MM-dd")}
				}
			...
			switch (obj.event) {
				case 'del-comment': {
					layer.confirm('确定删除吗？', function (index) {
						$.ajax({
							url: '<%=request.getContextPath()%>/delete/intoMan/intoManId',
							data: "intoId=" + data.intoId,
							method: 'post',
		...
		function updateMed(intoId) {
			var url = 'intoman-update.jsp?intoId='+intoId;
			x_admin_show('编辑', url);
		...
		function delAll() {
			...
			if (arr.length != 0) {
				layer.confirm('确认要删除吗？', function (index) {
					...
					//发异步删除数据
					$.ajax({
						url: "<%=request.getContextPath()%>/delete/intoMan/intoManIds",
						data: {'datas': arr},
						dataType: "json",
						async: false,
						type: "post",
