...
<div class="x-body">
    ...
    <xblock>
        <button class="layui-btn" onclick="x_admin_show('添加','./soldnote-add.jsp',600,400)"><i
...
<script>
    layui.use(['table', 'form', 'layer','jquery','util'], function () {
        ...
        table.render({
            ...
            , elem: '#comment-table'
            , cellMinWidth: 40
            , url: '<%=request.getContextPath()%>/select/soldNoteList/soldNote' //数据接口，查询所有list
            ...
            , cols: [[ //表头
                {type: 'checkbox'},
                {field: 'soldId', title: 'ID', sort: true, width: 80}//主键
                , {field: 'solderName', title: '销售员姓名'} //对应的实体的字段
                , {field: 'soldTime', title: '销售时间',templet: function (data) {
                        return  util.toDateString(data.soldTime, "yyyy-MM-dd")}}
                , {field: 'orderId', title: '订单编号'} //对应的实体的字段
                , {field: 'customerName', title: '顾客姓名'} //对应的实体的字段
            ...
            switch (obj.event) {
                case 'del-comment': {
                    layer.confirm('确定删除吗？', function (index) {
                        $.ajax({
                            url: '<%=request.getContextPath()%>/delete/soldNote/soldNoteId',
                            data: "soldId=" + data.soldId,
                            method: 'post',
        ...
        function updateMed(soldId) {
            var url = 'soldnote-update.jsp?soldId=' + soldId;
            x_admin_show('编辑', url);
        ...
        function delAll() {
            ...
            if (arr.length != 0) {
                layer.confirm('确认要删除吗？', function (index) {
                    ...
                    //发异步删除数据
                    $.ajax({
                        url: "<%=request.getContextPath()%>/delete/soldNote/soldNotes",
                        data: {'datas': arr},
                        dataType: "json",
                        async: false,
                        type: "post",
