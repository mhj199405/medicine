...
<div class="x-body">
	...
	<div class="layui-row">
		<form class="layui-form layui-col-md12 x-so">
			<input type="text" name="memberGarde" id="memberGarde" placeholder="请输入积分等级" autocomplete="off" class="layui-input"
				   lay-verify="required">
			<button class="layui-btn" lay-submit="" lay-filter="search-comment"><i class="layui-icon">&#xe615;</i></button>
		</form>
	</div>
	<xblock>
		<button class="layui-btn" onclick="x_admin_show('添加','./member-add.jsp',600,400)"><i
...
<script>
	layui.use(['table', 'form', 'layer','jquery','util'], function () {
		...
		table.render({
			...
			, elem: '#comment-table'
			, cellMinWidth: 40
			, url: '<%=request.getContextPath()%>/select/memberList/member' //数据接口，查询所有list
			...
			, cols: [[ //表头
				{type: 'checkbox'},
				{field: 'memberId', title: 'ID', sort: true,width:60}//主键
				, {field: 'memberGarde', title: '积分等级'} //对应的实体的字段
				, {field: 'memberLipin', title: '礼品'} //对应的实体的字段
				, {field: 'memberLasttime', title: '结束时间',width:110,templet: function (data) {
					return  util.toDateString(data.memberLasttime, "yyyy-MM-dd")}
				}
			...
			switch (obj.event) {
				case 'del-comment': {
					layer.confirm('确定删除吗？', function (index) {
						$.ajax({
							url: '<%=request.getContextPath()%>/delete/member/memberId',
		...
		function delAll() {
			...
			if (arr.length != 0) {
				layer.confirm('确认要删除吗？', function (index) {
					...
					//发异步删除数据
					$.ajax({
						url: "<%=request.getContextPath()%>/delete/member/members",
						data: {'datas': arr},
						dataType: "json",
						async: false,
						type: "post",
